...
For normal highlight and free-text rows, clicking a sidebar row can locate the annotation and expose the small PDF.js editor toolbar for actions such as recolor or delete.

For some persisted ink rows in the bundled sample PDF, clicking the sidebar row located the annotation on the page but did not expose the ink editor toolbar.

The confusing part was that all of these rows looked like "ink" in the sidebar. Some ink rows behaved like editable pen strokes, while other ink rows only behaved like locate targets.

The affected annotations in `static/sample.pdf` were:

- page 2, second ink row
- page 2, third ink row
- page 3, second ink row
...
The sidebar row label "Ink" was too broad.
...
- plain `/Ink` rows could activate an ink editor
- `/Ink` plus `/IT /InkHighlight` rows were classified by PDF.js as highlight editor targets

If the app blindly tries to activate every `/Subtype /Ink` row as an ink editor, some rows will fail. That failure is deterministic for this sample and this PDF.js version, not random timing.
...
2. Still scroll to and outline the annotation when the row is clicked.
...
- persisted ink row behavior is explainable from PDF object data
...
- some rows still look like ink but do not expose the ink editor toolbar
...
- Keep browser and native regression coverage for sidebar row clicking because PDF.js editor activation can differ between Chromium and WKWebView.
