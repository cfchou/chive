...
const bookmarkPdfPath = "/tmp/chive-native-bookmark.pdf";
...
describe("native WKWebView PDF smoke", () => {
  ...
  it("locates persisted ink rows independently after adjacent ink clicks", async () => {
    ...
    await app.execute(async (filePath) => {
      await window.__pdfSpike!.loadPath(filePath);
      document.querySelector<HTMLButtonElement>("[role='tab'][aria-label='Annotations']")?.click();
    }, samplePdfPath);
  ...
  it("handles missing and broken outline destinations in native WKWebView", async () => {
    ...
    const state = await app.execute(async () => {
      document.querySelector<HTMLButtonElement>("[role='tab'][aria-label='Outline']")?.click();
  ...
  it("creates, reloads, and navigates PDF-native bookmarks in native WKWebView", async () => {
    ...
    await app.execute(() => {
      const tab = (label: string) =>
        document.querySelector<HTMLButtonElement>(`[role='tab'][aria-label='${label}']`);
      tab("Outline")?.click();
    ...
    await app.execute(async () => {
      await window.__pdfSpike!.createBookmarkForCurrentPage();
    });
    await app.waitUntil(
      async () =>
    ...
    const openedEditor = await app.execute(() => {
      const row = document.querySelector<HTMLButtonElement>(".bookmark-title-button");
      if (!row) return false;
      row.dispatchEvent(new MouseEvent("dblclick", { bubbles: true, cancelable: true, composed: true }));
      return true;
    });
