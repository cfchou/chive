...
test("bookmark row can be renamed inline", async ({ page }) => {
  ...
  await expect(page.locator('.dirty-dot[aria-label="Unsaved changes"]')).toBeVisible();
});
...
test("existing bookmark can be edited for title and color", async ({ page }) => {
  ...
  await expect(page.locator(".bookmark-color-chip")).toHaveCount(1);
...
test("bookmark color can be edited and persists through native outline color", async ({ page }) => {
  ...
  await expect(page.getByRole("button", { name: "Set bookmark color pink" })).toHaveCount(0);
