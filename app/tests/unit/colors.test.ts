...
describe("PDF color helpers", () => {
  ...
  it("shares the bookmark and outline row palette without green", () => {
    assert.deepEqual(bookmarkPalette, outlinePalette);
    assert.deepEqual(
  ...
  it("defines the eight annotation palette colors and preserves key highlight literals", () => {
    assert.deepEqual(
      annotationPalette.map((option) => option.name),
      ["red", "orange", "yellow", "green", "cyan", "blue", "purple", "rose"],
    );
    assert.equal(annotationPalette.find((option) => option.name === "yellow")?.highlightValue, "#fff35c");
    assert.equal(annotationPalette.find((option) => option.name === "green")?.highlightValue, "#7cf2aa");
    assert.equal(annotationPalette.find((option) => option.name === "blue")?.highlightValue, "#8ecbff");
    assert.equal(annotationPalette.find((option) => option.name === "rose")?.highlightValue, "#ffb6de");
  });
});
